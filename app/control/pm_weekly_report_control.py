...
class PMWeeklyReportControl:
    ...
    def _to_dt(self, v: Any) -> Optional[datetime]:
        """容错把可能的 str/datetime 转为 UTC datetime"""
        if v is None or v == "":
            return None
    ...
    def _r_owner_name(self, r: Any) -> str:
        # 优先直接取用户名字段；没有则尝试用 user_repo 反查
    ...
    # ----------------- 页面用主方法 -----------------
    def list_last_7d(self) -> List[Dict[str, Any]]:
    ...
    def generate(self):
        ...
        items = self._request_repo.list_matched_since(since)  # 你现有的取数逻辑
        title = "Weekly Matched Report (Last 7 days)"

        report = Report(
            id=None,
            title=title,
            period="weekly",  # weekly / monthly 对应填 "weekly"/"monthly"
