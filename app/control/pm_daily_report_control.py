...
class PMDailyReportControl:
    ...
    # ----------------- 页面用主方法 -----------------
    def list_last_24h(self) -> List[Dict[str, Any]]:
        """
        返回最近 24 小时内完成匹配的请求（按 matched_at 倒序）
        """
        now = datetime.utcnow()
    ...
    def generate(self):
        ...
        title = "Daily Matched Report (Last 24 hours)"

        report = Report(
            id=None,
            title=title,
            period="daily",  # weekly / monthly 对应填 "weekly"/"monthly"
